...
resource "google_compute_health_check" "mig-https-health-check" {
  count   = "${var.module_enabled && var.health_check_type == "HTTPS" ? 1 : 0}"
  name    = "${var.name}"
  project = "${var.project}"
...
resource "google_compute_health_check" "mig-http-health-check" {
  count   = "${var.module_enabled && var.health_check_type == "HTTP" ? 1 : 0}"
  name    = "${var.name}"
  project = "${var.project}"
...
resource "google_compute_firewall" "mig-health-check" {
  count   = "${var.module_enabled && var.health_check_type == "" ? 0 : 1}"
